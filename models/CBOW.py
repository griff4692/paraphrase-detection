...
class CBOW(nn.Module):
    def __init__(self, vocab, embed_dim, hidden_dim = 128):
        super(CBOW, self).__init__()
        self.vocab = vocab
        self.embeddings = nn.EmbeddingBag(vocab.size() + 1, embed_dim, mode='sum')
    ...
    def prepare_batch(self, batch_sentences1, batch_sentences2, labels):
        ...
        for i in range(batch_size):
            tokens1 = self.vocab.sentence_to_idxs(batch_sentences1[i])
            tokens1_len = len(tokens1)

            tokens2 = self.vocab.sentence_to_idxs(batch_sentences2[i])
